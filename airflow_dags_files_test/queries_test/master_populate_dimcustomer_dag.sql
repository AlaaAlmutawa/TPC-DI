...
--INSERT INTO `master1.DimCustomer`
SELECT 
...
FROM
staging1.customer_historical c 
LEFT JOIN master1.TaxRate n ON (c.NationalTaxID = n.TX_ID)
LEFT JOIN master1.TaxRate l ON (c.LocalTaxID = l.TX_ID)
LEFT JOIN staging1.prospect p ON (UPPER(p.LastName) = UPPER(c.LastName) 
