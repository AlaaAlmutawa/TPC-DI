...
watch_hist as
  (
    select
        ...
        W_ACTION
    from staging1.watch_history wh
    join master1.DimCustomer cust on wh.W_C_ID=cust.CustomerID
        and (wh.W_DTS)>= cust.EffectiveDate 
        and (wh.W_DTS)<= cust.EndDate
    join master1.DimSecurity s on wh.W_S_SYMB=s.Symbol
        and (wh.W_DTS)>= s.EffectiveDate 
        and (wh.W_DTS)<= s.EndDate
    join master1.DimDate d on DATE(wh.W_DTS) = 	d.DateValue
